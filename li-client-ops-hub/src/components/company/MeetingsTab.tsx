        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-7 pr-2 py-1 text-xs border border-slate-200 rounded-md focus:border-teal-500 focus:outline-none"
          ...
          {display.map((turn, i) => {
            ...
            return (
              <div key={i} className="flex gap-2 py-0.5">
                {ts && <span className="text-slate-400 w-10 shrink-0 font-mono text-[10px]">{ts}</span>}
                <span className="font-medium text-slate-700 shrink-0">{turn.speaker?.name || 'Speaker'}:</span>
                <span className="text-slate-600">{highlightSearch(turn.text, searchQuery)}</span>
              </div>
            );
          })}
        </div>
    ...
    <div className="space-y-1">
      {meeting.recording_local_path ? (
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs text-teal-600 hover:text-teal-800 disabled:opacity-50"
        >
          <Download size={12} />
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-3 py-2.5 text-sm font-medium text-slate-900">{meeting.title ?? 'Untitled'}</td>
        <td className="px-3 py-2.5 text-xs text-slate-600">
          {meeting.duration_minutes != null ? `${meeting.duration_minutes}m` : '-'}
        </td>
        <td className="px-3 py-2.5 text-xs text-slate-500">{meeting.platform ?? '-'}</td>
        <td className="px-3 py-2.5 text-xs text-slate-600">{meeting.attended_count}/{meeting.participants_count}</td>
        <td className="px-3 py-2.5">
          {meeting.read_score != null ? (
            ...
            <span className="text-xs text-slate-400">{meeting.expanded ? '-' : 'Pending'}</span>
          )}
        </td>
      </tr>
        ...
        <tr>
          <td colSpan={7} className="bg-slate-50 px-6 py-4">
            <div className="space-y-4">
              ...
              {meeting.summary && (
                <div>
                  <h4 className="text-xs font-semibold uppercase text-slate-500 mb-1">Summary</h4>
              ...
              {chapterSummaries.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold uppercase text-slate-500 mb-1">Chapter Summaries</h4>
                  ...
                  <div className="space-y-1">
                    {actionItems.map((item) => (
                      <div key={item.id} className="flex items-start gap-2 text-xs">
                        ...
                        <span className={`text-slate-700 ${item.status === 'done' ? 'line-through text-slate-400' : ''}`}>
                          {item.text}
                        </span>
                        {item.assignee && <span className="shrink-0 text-slate-400">(assigned: {item.assignee})</span>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {keyQuestions.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold uppercase text-slate-500 mb-1">Key Questions</h4>
                  <ul className="list-disc pl-4 space-y-0.5">
                    {keyQuestions.map((q, i) => (
                      <li key={i} className="text-xs text-slate-700">{typeof q === 'string' ? q : JSON.stringify(q)}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {topics.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold uppercase text-slate-500 mb-1">Topics</h4>
                  <div className="flex flex-wrap gap-1">
                    {topics.map((t, i) => (
                      <span key={i} className="rounded bg-slate-200 px-1.5 py-0.5 text-[10px] text-slate-600">
              ...
              {participants.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold uppercase text-slate-500 mb-1">Participants</h4>
                  <div className="space-y-0.5">
                    {participants.map((p, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-slate-700">{p.name ?? p.email ?? 'Unknown'}</span>
                        {p.email && <span className="text-slate-400">{p.email}</span>}
                        {p.attended === false && p.invited && <span className="text-slate-400 italic">invited, not attended</span>}
                      </div>
                    ))}
                  </div>
                </div>
...
export default function MeetingsTab({ companyId }: { companyId: string }) {
  ...
  if (loading) return <p className="p-4 text-sm text-slate-400">Loading meetings...</p>;
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 border-b border-slate-100 px-4 py-3">
        <div className="text-center">
          <div className="text-lg font-semibold text-slate-900">{last30.length}</div>
          <div className="text-[10px] text-slate-400">Meetings (30d)</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-semibold text-slate-900">{avgDuration}m</div>
        ...
        <div className="text-center">
          <div className="text-lg font-semibold text-slate-900">{expandedCount}/{meetings.length}</div>
          <div className="text-[10px] text-slate-400">Expanded</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-semibold text-slate-900">{withTranscript}</div>
      ...
      <table className="w-full text-left text-sm">
        <thead className="border-b border-slate-200 bg-slate-50">
          <tr>
            ...
            <th className="px-3 py-2 text-xs font-medium uppercase text-slate-500">Date</th>
            <th className="px-3 py-2 text-xs font-medium uppercase text-slate-500">Title</th>
            <th className="px-3 py-2 text-xs font-medium uppercase text-slate-500">Duration</th>
            <th className="px-3 py-2 text-xs font-medium uppercase text-slate-500">Platform</th>
            <th className="px-3 py-2 text-xs font-medium uppercase text-slate-500">Attendees</th>
            <th className="px-3 py-2 text-xs font-medium uppercase text-slate-500">Score</th>
          </tr>
        </thead>
        <tbody>
          {meetings.map((m) => (
